...
@section('body')
    <div class="page-wrapper">
        <div class="content container-fluid">
            <!-- <div>Hello From view_of_faculty_for_video_type_tutorial</div><br> -->

            @if(Session()->has('delete_video_tutorial_from_faculty_route_msg'))

                <div class="msg" id="msg1">
                    {{session()->get('delete_video_tutorial_from_faculty_route_msg')}}
                    <button type="button" onclick = "document.getElementById('msg1').style.display = 'none';" id="popup-btn" >X</button>
                </div>

            @endif

            @if(count($particular_faculty_all_video_tutorials) > 0)
                <div class="center">
                    <h2 id="main-heading">All Video Tutorials You Added</h2>
                    ...
                    <div class="table_container">
                        <table id="cat_table">
                            ...
                            @foreach($particular_faculty_all_video_tutorials as $each_video_tutorial)
                                <tr id="all_rows">
                                    <td>{{ $srNo++ }}</td>  
                                    ...
                                    <td><a onclick="return confirm('Are you sure to Update selected Video Tutorial !')" href="{{ route('updating_video_tutorial_from_faculty_page', $each_video_tutorial->id) }}" id="edit_btn">Edit</a></td>
                                    <td><a onclick="return confirm('Are you sure to Delete selected Video Tutorial !')" href="{{ route('deleting_video_tutorial_from_faculty', $each_video_tutorial->id) }}" id="delete_btn">Delete</a></td> 
                                </tr>
                            @endforeach
            ...
            @else
                <script>
                    window.location.href = '{{ route('main_faculty_dashboard_nothing_to_show_in_video_type_tutorial') }}';
                </script>
            @endif
