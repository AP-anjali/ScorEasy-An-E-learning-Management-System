...
<head>
   ...
   <title>{{ $faculty_data->name }} | Profile</title>
...
<script>
    ...
    function checkFormChanges(){
        let name = "{{ $faculty_data->name }}";
        let email = "{{ $faculty_data->email }}";
        let phone_no = "{{ $faculty_data->phone_no }}";
        let date_of_birth = "{{ $faculty_data->date_of_birth }}";
        let original_profile_pic = "{{ $faculty_data->profile_pic }}";

        let username = "{{ $faculty_data->username }}";
        let confirm_password = "{{ $faculty_data->confirm_password }}";

        let bank_name = "{{ $faculty_data->bank_name }}";
        let bank_branch = "{{ $faculty_data->bank_branch }}";
        let bank_ifsc_code = "{{ $faculty_data->bank_ifsc_code }}";
        let bank_micr_code = "{{ $faculty_data->bank_micr_code }}";
        let account_holder_name = "{{ $faculty_data->account_holder_name }}";
        let account_number = "{{ $faculty_data->account_number }}";
        let account_type = "{{ $faculty_data->account_type }}";
        let original_bank_document = "{{ $faculty_data->proof_of_bank_account_ownership_file_path }}";
        ...
        && input_username === username && input_confirm_password === confirm_password && input_account_type === account_type
...
<section class="contact"> <!-- customer_account -->
    ...
    @if(Session()->has('faculty_record_to_update'))
        <div class="msg" id="msg1">
            {{session()->get('faculty_record_to_update')}}
    ...
    <div class="customer_account_main_container">
        ...
        <form action="{{ route('updating_faculty_user_record_from_faculty_page_form_submission', $faculty_data->id ) }}" method = "post" enctype="multipart/form-data" id="accountDetailsForm">
            @csrf
            ...
            <div class="customer_account_image_slot_container">
                <img src="{{ asset('storage/' . $faculty_data->profile_pic) }}" alt="User Profile Pic">
            ...
            <div class="users_info_container">
                ...
                <div class="button-container">
                    <div class="text-fields name">
                        ...
                        <input type="text" name="name" id="name" value = "{{ $faculty_data->name }}" placeholder="Your full name" required oninput="checkFormChanges()">
                    </div>
                    <div class="text-fields email">
                        <label for="email"><i class='bx bx-envelope bx-md' style='color:#6487cb'></i></label>
                        <input type="text" name="email" id="email" value = "{{ $faculty_data->email }}" placeholder="Your Email Address" required oninput="checkFormChanges()">
                    </div>
                </div>
                ...
                <div class="button-container">
                    <div class="text-fields phone_no">
                        ...
                        <input type="text" name="phone_no" id="phone_no" value = "{{ $faculty_data->phone_no }}" placeholder="Phone number with +91" required oninput="checkFormChanges()">
                    </div>
                    <div class="text-fields date_of_birth">
                        <label for="date_of_birth"><i class='bx bxl-periscope bx-md' style='color:#6487cb' ></i></label>
                        <input type="date" name="date_of_birth" id="date_of_birth" value = "{{ $faculty_data->date_of_birth }}" placeholder="Your Business address" required oninput="checkFormChanges()">
                    </div>
                </div>        
                    ...
                    <div id="secured_data_that_need_user_verification">
                        ...
                        <div class="button-container">
                            <div class="text-fields username">
                                ...
                                <input type="text" name="username" id="username" value = "{{ $faculty_data->username }}" placeholder="Create Own Username" required oninput="checkFormChanges()">
                            </div>

                            <div class="text-fields confirm_password">
                                <label for="confirm_password"><i class='bx bxs-key bx-md' style='color:#6487cb' ></i></label>
                                <input type="password" name="confirm_password" id="confirm_password" value = "{{ $faculty_data->confirm_password }}" placeholder="Enter Password" required oninput="checkFormChanges()">
                            ...
                            <h2>Banking Information</h2>
                                        ...
                                        <div class="button-container">
                                            <div class="text-fields bank_name" >
                                                ...
                                                <input type="text" name="bank_name" id="bank_name" value = "{{ $faculty_data->bank_name }}" required oninput="checkFormChanges()">
                                            </div>
                                            <div class="text-fields bank_branch" >
                                                <label for="bank_branch"><i class='bx bxs-doughnut-chart bx-md' style='color:#6487cb'></i></label>
                                                <input type="text" name="bank_branch" id="bank_branch" value = "{{ $faculty_data->bank_branch }}" required oninput="checkFormChanges()">
                                            </div>
                                        </div>
                                        <div class="button-container">
                                            <div class="text-fields bank_ifsc_code" >
                                                <label for="bank_ifsc_code"><i class='bx bxs-bank bx-md' style='color:#6487cb'></i></label>
                                                <input type="text" name="bank_ifsc_code" id="bank_ifsc_code" value="{{ $faculty_data->bank_ifsc_code }}" placeholder="Bank Account IFSC Code" required oninput="checkFormChanges()">
                                            </div>
                                            <div class="text-fields bank_micr_code" >
                                                ...
                                                <input type="text" name="bank_micr_code" id="bank_micr_code" value="{{ $faculty_data->bank_micr_code }}" placeholder="Bank Account MICR Code" required oninput="checkFormChanges()">
                                            </div>
                                        </div>
                                        <div class="button-container">
                                            <div class="text-fields account_holder_name">
                                                ...
                                                <input type="text" name="account_holder_name" id="account_holder_name" value = "{{ $faculty_data->account_holder_name }}" required oninput="checkFormChanges()">
                                            </div>
                                            <div class="text-fields account_number">
                                                ...
                                                <input type="text" name="account_number" id="account_number" value = "{{ $faculty_data->account_number }}" required oninput="checkFormChanges()">
                                            </div>
                                        </div>
                                        <div class="button-container">
                                            ...
                                            <div class="text-fields account_type">
                                                ...
                                                <select name="account_type" id="my_list4" required onchange="checkFormChanges()">
                                                    <option value="{{ $faculty_data->account_type }}" selected>{{ $faculty_data->account_type }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</option>
                                            ...
                                            <div class="file_uploader">
                                                ...
                                                <input type="file" name="proof_of_bank_account_ownership_file_path" id="proof_of_bank_account_ownership" onchange="checkFormChanges();">
                                            </div>
                                        ...
                                        <div class="button-container">
                                            <div class = "customer_account_current_bank_document_btn">
                                                <a href = "{{ route('showing_proof_of_bank_account_ownership_to_faculty_for_account', $faculty_data->id) }}" target = "_blank">CURRENT PROOF OF BANK ACCOUNT OWNERSHIP</a>
                                            </div>
                                        </div>
                        ...
                        <div class="button-container customer_account_creation_info_slot" style = "margin-top: 60px;">
                            ...
                            <div class = "customer_account_creation_date">
                                <h2><?php echo date("d/m/Y, h:i:s A", strtotime($faculty_data->registration_date)); ?></h2>
                            </div>
                        </div>
        ...
        <div class="users_info_container">
            ...
            <div class="verify_user_for_secured_data" style="{{ Session::has('verification_code_correct') ? 'display: none;' : '' }}">
                ...
                <button class = "user_email_verification_btn" onclick = "goToRouteToSentVerificationCode({{ $faculty_data->id }})" style="{{ Session::has('mail_has_been_sent_to_user') ? 'pointer-events: none; opacity: 0.5;' : '' }}">EMAIL VERIFICATION</button>
            </div>
            ...
            @if(Session::has('mail_has_been_sent_to_user'))
                <div id="email_verification_form_that_will_be_dispalyed_when_an_email_has_been_sent">
                    <form action="{{ route('verifying_faculty_email_verification_code_form_submission', $faculty_data->id) }}" method = "post">
                        @csrf
                        <div class="email_text_box_container">
                            <div class="email_v_text">We have sent a verification code on <span class = "users_mail">"{{ $faculty_data->email }}"</span></div>
                            <div class="button-container">
            ...
            <div class="change_reset_btn_slot">
                <div class="button-container">
                    <div class = "customer_account_deletion_btn disabled_due_to_checkbox_data" id = "id_for_enable_desable_delete_account_btn">
                        <a onclick = "return confirm('Are You Sure To Delete Your Account !');" href = "{{ route('deleting_faculty_account_from_faculty', $faculty_data->id) }}" id="deleteButton">DELETE ACCOUNT</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    {
        var redirecting_route = "/sending_email_for_verify_faculty_user/" + userID;
